  ...
  return (
    <Shell user={user}>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <Link href="/care" className="text-primary-700 font-semibold">← Пациенты</Link>
      ...
      {alerts.filter((a) => !a.acknowledged).length > 0 && (
        <div className="card !bg-danger-500/5 border border-danger-500/20 mb-4">
          <div className="font-bold text-danger-500 mb-2">
            Активные оповещения ({alerts.filter((a) => !a.acknowledged).length})
          </div>
      ...
      <h2 className="text-xl font-bold mb-2">Последние показатели</h2>
      <SummaryGrid metrics={summary} />

      <h2 className="text-xl font-bold mt-6 mb-2">Динамика</h2>
