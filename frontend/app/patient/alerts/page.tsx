...
export default function AlertsPage() {
  ...
  return (
    <Shell user={user}>
      <h1 className="text-2xl font-bold mb-4">Оповещения</h1>
      <div className="space-y-2">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {a.acknowledged && <span className="badge-ok">Просмотрено</span>}
                  </div>
                  <div className="text-ink-700 mt-1">{a.reason}</div>
                  {a.value != null && (
                    <div className="text-sm text-ink-500 mt-1">
                      Значение: <b>{formatValue(a.kind as MetricKind, a.value)} {meta?.unit}</b>
        ...
        {alerts.length === 0 && <div className="card text-ink-500">Всё в порядке — оповещений нет.</div>}
      </div>
    </Shell>
  );
}
