...
func runSweep(ctx context.Context, pool *pgxpool.Pool) {
	...
	defer cancel()
	// Build the interval inline: AlgorithmRunsRetentionDays is a code
	// constant, never user input, so this is not an injection risk and
	// avoids pgx's text-vs-interval encoding pickiness.
	tag, err := pool.Exec(sweepCtx, fmt.Sprintf(
		`DELETE FROM algorithm_runs WHERE created_at < now() - interval '%d days'`,
		AlgorithmRunsRetentionDays,
	))
	if err != nil {
		log.Printf("retention: algorithm_runs sweep failed: %v", err)
		return
	}
	if n := tag.RowsAffected(); n > 0 {
		log.Printf("retention: pruned %d algorithm_runs rows older than %d days",
			n, AlgorithmRunsRetentionDays)
	}
}
